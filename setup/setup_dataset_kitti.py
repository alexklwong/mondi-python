...
TRAIN_GROUND_TRUTH_LEFT_FILEPATH = os.path.join(
    TRAIN_REF_DIRPATH, 'kitti_train_ground_truth_left.txt')
TRAIN_GROUND_TRUTH_RIGHT_FILEPATH = os.path.join(
    TRAIN_REF_DIRPATH, 'kitti_train_ground_truth_right.txt')
...
# Paths to files containing nonstatic data
...
TRAIN_NONSTATIC_GROUND_TRUTH_LEFT_FILEPATH = os.path.join(
    TRAIN_REF_DIRPATH, 'kitti_train_nonstatic_ground_truth_left.txt')
TRAIN_NONSTATIC_GROUND_TRUTH_RIGHT_FILEPATH = os.path.join(
    TRAIN_REF_DIRPATH, 'kitti_train_nonstatic_ground_truth_right.txt')
...
UNUSED_GROUND_TRUTH_LEFT_FILEPATH = os.path.join(
    TRAIN_REF_DIRPATH, 'kitti_unused_ground_truth_left.txt')
UNUSED_GROUND_TRUTH_RIGHT_FILEPATH = os.path.join(
    TRAIN_REF_DIRPATH, 'kitti_unused_ground_truth_right.txt')
UNUSED_FOCAL_LENGTH_BASELINE_LEFT_FILEPATH = os.path.join(
    TRAIN_REF_DIRPATH, 'kitti_unused_focal_length_baseline_left.txt')
UNUSED_FOCAL_LENGTH_BASELINE_RIGHT_FILEPATH = os.path.join(
    TRAIN_REF_DIRPATH, 'kitti_unused_focal_length_baseline_right.txt')
...
def setup_dataset_kitti_training(paths_only=False, n_thread=8):
    ...
    train_sparse_depth_left_paths = []
    train_sparse_depth_right_paths = []
    train_ground_truth_left_paths = []
    train_ground_truth_right_paths = []
    ...
    train_nonstatic_sparse_depth_left_paths = []
    train_nonstatic_sparse_depth_right_paths = []
    train_nonstatic_ground_truth_left_paths = []
    train_nonstatic_ground_truth_right_paths = []
    ...
    for refdir in ['train', 'val']:
        ...
        for sequence_dirpath in sparse_depth_sequence_dirpath:
            ...
            keep_indices = []
            for idx in range(len(image_left_paths)):

                filename_left = image_left_paths[idx].split(os.sep)[-1]
                filename_right = image_right_paths[idx].split(os.sep)[-1]
            ...
            image_left_paths = [
                image_left_paths[idx] for idx in keep_indices
            ]
            image_left_paths = sorted(image_left_paths)

            image_right_paths = [
                image_right_paths[idx] for idx in keep_indices
            ]
            ...
            if refdir == 'train':
                ...
                for image_idx, data_paths in zip([0, 1], [data_left_paths, data_right_paths]):
                    image_paths, \
                        sparse_depth_paths, \
                        ground_truth_paths, \
                        focal_length_baseline_paths, \
                    ...
                    pool_inputs = []
                    for idx in range(1, len(sparse_depth_paths)-1):
                    ...
                    with mp.Pool(n_thread) as pool:
                        ...
                        for result in pool_results:
                            ...
                            if image_idx == 0:
                                ...
                                train_sparse_depth_left_paths.append(sparse_depth_path)
                                train_ground_truth_left_paths.append(ground_truth_path)
                            ...
                            # Save paths for right images
                                ...
                                train_sparse_depth_right_paths.append(sparse_depth_path)
                                train_ground_truth_right_paths.append(ground_truth_path)
    ...
    train_sparse_depth_left_paths = sorted(train_sparse_depth_left_paths)
    train_sparse_depth_right_paths = sorted(train_sparse_depth_right_paths)
    train_ground_truth_left_paths = sorted(train_ground_truth_left_paths)
    train_ground_truth_right_paths = sorted(train_ground_truth_right_paths)
    ...
    train_nonstatic_images_left_paths, \
        ...
        train_nonstatic_ground_truth_left_paths, \
        train_nonstatic_focal_length_baseline_left_paths, \
        train_nonstatic_intrinsics_left_paths = filter_static_frames(train_data_left_paths)
    ...
    train_nonstatic_images_right_paths, \
        ...
        train_nonstatic_ground_truth_right_paths, \
        train_nonstatic_focal_length_baseline_right_paths, \
        train_nonstatic_intrinsics_right_paths = filter_static_frames(train_data_right_paths)

    # Sanity checks
    n_nonstatic_paths = len(train_nonstatic_images_left_paths) + \
        len(train_nonstatic_images_right_paths)

    assert n_nonstatic_paths == len(train_nonstatic_sparse_depth_left_paths) + \
        len(train_nonstatic_sparse_depth_right_paths)
    assert n_nonstatic_paths == len(train_nonstatic_ground_truth_left_paths) + \
        len(train_nonstatic_ground_truth_right_paths)
    assert n_nonstatic_paths == len(train_nonstatic_focal_length_baseline_left_paths) + \
        len(train_nonstatic_focal_length_baseline_right_paths)
    assert n_nonstatic_paths == len(train_nonstatic_intrinsics_left_paths) + \
    ...
    '''
    train_filepaths = [
        (train_images_left_paths, TRAIN_IMAGES_LEFT_FILEPATH),
        (train_images_right_paths, TRAIN_IMAGES_RIGHT_FILEPATH),
        (train_sparse_depth_left_paths, TRAIN_SPARSE_DEPTH_LEFT_FILEPATH),
        (train_sparse_depth_right_paths, TRAIN_SPARSE_DEPTH_RIGHT_FILEPATH),
        (train_ground_truth_left_paths, TRAIN_GROUND_TRUTH_LEFT_FILEPATH),
        (train_ground_truth_right_paths, TRAIN_GROUND_TRUTH_RIGHT_FILEPATH),
        (train_focal_length_baseline_left_paths, TRAIN_FOCAL_LENGTH_BASELINE_LEFT_FILEPATH),
        (train_focal_length_baseline_right_paths, TRAIN_FOCAL_LENGTH_BASELINE_RIGHT_FILEPATH),
        (train_intrinsics_left_paths, TRAIN_INTRINSICS_LEFT_FILEPATH),
        (train_intrinsics_right_paths, TRAIN_INTRINSICS_RIGHT_FILEPATH)
    ]

    train_nonstatic_filepaths = [
        (train_nonstatic_images_left_paths, TRAIN_NONSTATIC_IMAGES_LEFT_FILEPATH),
        (train_nonstatic_images_right_paths, TRAIN_NONSTATIC_IMAGES_RIGHT_FILEPATH),
        (train_nonstatic_sparse_depth_left_paths, TRAIN_NONSTATIC_SPARSE_DEPTH_LEFT_FILEPATH),
        (train_nonstatic_sparse_depth_right_paths, TRAIN_NONSTATIC_SPARSE_DEPTH_RIGHT_FILEPATH),
        (train_nonstatic_ground_truth_left_paths, TRAIN_NONSTATIC_GROUND_TRUTH_LEFT_FILEPATH),
        (train_nonstatic_ground_truth_right_paths, TRAIN_NONSTATIC_GROUND_TRUTH_RIGHT_FILEPATH),
        (train_nonstatic_focal_length_baseline_left_paths, TRAIN_NONSTATIC_FOCAL_LENGTH_BASELINE_LEFT_FILEPATH),
        (train_nonstatic_focal_length_baseline_right_paths, TRAIN_NONSTATIC_FOCAL_LENGTH_BASELINE_RIGHT_FILEPATH),
        (train_nonstatic_intrinsics_left_paths, TRAIN_NONSTATIC_INTRINSICS_LEFT_FILEPATH),
        (train_nonstatic_intrinsics_right_paths, TRAIN_NONSTATIC_INTRINSICS_RIGHT_FILEPATH)
    ]
